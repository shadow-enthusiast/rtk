...
type TextContentBlock = { type: "text"; text: string; [key: string]: unknown };
type ToolResult = { content?: unknown; details?: unknown; [key: string]: unknown };

function getTextBlocks(result: ToolResult): TextContentBlock[] | null {
  ...
  for (const block of blocks) {
    ...
    textBlocks.push(candidate as TextContentBlock);
  }
  return textBlocks;
}
...
function shouldAttemptResultCompression(text: string) {
  return text.length >= finitePositive(RESULT_MIN_CHARS, 12000);
}

function withCompactedText(result: ToolResult, blocks: TextContentBlock[], text: string): ToolResult {
  if (blocks.length === 1) {
...
function maybeAcceptCompaction(original: string, compacted: string, label: string) {
...
async function runRtk(args: string[], input?: string) {
  if (input === undefined) {
...
function isPlainRead(event: { args?: Record<string, unknown> }) {
...
async function compactReadResult(event: { args?: Record<string, unknown>; result: ToolResult }, original: string) {
  if (!isPlainRead(event)) return null;
  const args = event.args || {};
  const rtkArgs = ["read", "--level", READ_LEVEL, String(args.path)];
  const output = await runRtk(rtkArgs);
  return maybeAcceptCompaction(original, output, "read result via rtk read");
}

async function compactProcessResult(event: { args?: Record<string, unknown>; result: ToolResult }, original: string) {
...
async function compactToolResult(event: { toolName: string; args?: Record<string, unknown>; isError?: boolean; result: ToolResult }) {
...
export default function register(api: any) {
  ...
  if (typeof api.registerAgentToolResultMiddleware === "function") {
    api.registerAgentToolResultMiddleware(
      async (event: { toolName: string; args?: Record<string, unknown>; isError?: boolean; result: ToolResult }) => {
        try {
